...
```{r project.setup, echo=TRUE, include=TRUE, error=FALSE, results=FALSE, message=FALSE}
#Import libraries
library(tidyverse)
library(lubridate)
library(here)
...
\newpage
```{r Ozone 2018 Boxplot, echo=FALSE, fig.cap='2018 monthly Ozone levels', fig.height=3}

O3.18<-filter(epa_data,Year=='2018')%>%
    drop_na(Ozone)

O3.18$Month<-factor(O3.18$Month,levels=1:12,labels=month.abb)
...
```{r Ozone 2019 Boxplot, echo=FALSE, fig.cap='2019 monthly Ozone levels', fig.height=3}

O3.19<-filter(epa_data,Year=='2019')%>%
    drop_na(Ozone)

O3.19$Month<-factor(O3.19$Month,levels=1:12,labels=month.abb)
...
I did not use AI on this assignment. I don't like AI. Sara Bassett 
10/20/25
